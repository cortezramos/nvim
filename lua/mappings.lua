...
map("n", "<leader>qs", function() require("persistence").load() end, { desc = "Restaurar sesión" })

-- Restaurar la ÚLTIMA sesión global (la última que cerraste)
map("n", "<leader>ql", function() require("persistence").load({ last = true }) end, { desc = "Restaurar última sesión" })
