...
-- NOTA: jdtls se configura en ftplugin/java.lua para evitar conflictos
...
local on_attach_custom = function(client, bufnr)
  ...
  -- Forzamos el soporte de documentHighlight para illuminate
  ...
  print(string.format("LSP attached: %s | documentHighlight: %s", 
    client.name, 
    tostring(client.server_capabilities.documentHighlightProvider)))
end

-- VTSLS: TypeScript Language Server mejorado con soporte nativo para plugins de Vue
lspconfig.vtsls.setup {
  ...
  -- CRÍTICO: Incluir 'vue' para Hybrid Mode
  filetypes = {
    "typescript",
    "javascript",
    "javascriptreact",
  ...
  settings = {
    vtsls = {
      -- Plugin de Vue para soporte de archivos .vue
      tsserver = {
        globalPlugins = {
          {
...
-- VOLAR: Vue Language Server (v3.x) - SOLO maneja CSS/HTML
-- TypeScript es manejado por vtsls con el plugin de Vue
lspconfig.volar.setup {
  on_attach = on_attach_custom,
  capabilities = nvlsp.capabilities,
  
  -- CRÍTICO: Este on_init es necesario para que volar 3.x coopere con vtsls
  on_init = function(client)
    ...
    local function typescriptHandler(_, result, context)
      ...
      if not ts_client then
        -- Esperar a que vtsls se adjunte (hasta 1 segundo)
        if retries <= 10 then
          retries = retries + 1
  ...
  -- SOLO archivos .vue (Hybrid Mode automático en v3.x)
  filetypes = { "vue" },
}
